...
Core_autor_text = f"{Core_author}. Reproducible from public DK-RD2 code."
...
def compute_alpha_DK_from_DESI(
    desi_bao_mean_path: str,
    desi_bao_cov_path: str,
):
    """
    Compute alpha_DK and r_d from the same DESI BAO + CC calibration
    used in: What Is Gravity paper
    """

    from scipy.optimize import minimize

    Core_H0_eff = float(Core_Hubble_H0)
    Omega_m_LCDM_eff = float(Core_OMEGA_M_LCDM)
    Omega_L_LCDM_eff = float(Core_OMEGA_L_LCDM)
    ...
    def H_DK_alpha(z_in, alpha_DK):
        return float(Core_H0_eff) * E_DK_alpha(z_in, alpha_DK)
    ...
    def chi2_dk_total(x):
        ...
        return float(chi2_bao + chi2_cc)

    x0 = np.array([
        0.5 * (float(rd_bounds[0]) + float(rd_bounds[1])),
        0.5 * (float(alpha_bounds[0]) + float(alpha_bounds[1])),
    ])

    res = minimize(
        chi2_dk_total,
        x0=x0,
        bounds=[rd_bounds, alpha_bounds],
        method="L-BFGS-B",
    )

    rd_dk_best = float(res.x[0])
    alpha_dk_best = float(res.x[1])

    return alpha_dk_best, rd_dk_best
...
# =============================================================================
# DK-RD2 inverse Friedmann closure: infer v(z), Gab(z), and w_eff(z)
# =============================================================================
#
# Thermal background:
#   T(z) = T0 (1 + z)
#
# Modified Friedmann relation:
#   E^2(z) = Ω_{m,0} (1+z)^3 [ 1 + β^2(z) * (T0 / T(z)) ]
#
# Inversion (no free parameters):
#   β^2(z) = (T(z) / T0) * [ E^2(z) / (Ω_{m,0} (1+z)^3) - 1 ]
#
# Velocity reconstruction:
#   v(z) = c * sqrt( β^2(z) )
#
# Thermodynamic gravitational coupling:
#   G_ab(z) = G0 [ 1 + β^2(z) * (T0 / T(z)) ]
#
# Effective dark-energy sector (diagnostic):
#   Ω_DE_eff(z) = E^2(z) - Ω_m (1+z)^3
#
# Effective equation of state:
#   w_eff(z) = -1 + (1+z)/3 * d/dz [ ln Ω_DE_eff(z) ]
#
# Notes:
# - v(z) is NOT a free parameter: it is reconstructed from E(z).
# - This closes DK-RD2 without phenomenological ansatz.
# - Ω_DE_eff and w_eff are emergent, not fundamental components.
# =============================================================================

def T_cosmic_z(z, T0: float = Core_TCMB_K):
    """
    Cosmic thermal background:
        T(z) = T0 * (1 + z)
    """
    z = np.asarray(z, dtype=float)
    return float(T0) * (1.0 + z)

def beta2_from_modified_friedmann(
    z,
    E_values,
    Omega_m0: float | None = None,
    T_model=None,
    ...
    """
    Infer beta^2(z) = v^2/c^2 from the DK-RD2 modified Friedmann relation.

    Starting point:
        E^2(z) = Omega_m0 * (1+z)^3 * [1 + beta^2(z) * T0/T(z)]

    Therefore:
        beta^2(z) = [T(z)/T0] * [ E^2(z)/(Omega_m0*(1+z)^3) - 1 ]

    No velocity ansatz is imposed here.
    """
    z = np.asarray(z, dtype=float)
    ...
    Tz = T_cosmic_z(z) if T_model is None else np.asarray(T_model(z), dtype=float)

    denom = Om0 * (1.0 + z) ** 3
    ratio = (E_values ** 2) / np.clip(denom, 1e-300, None)

    beta2 = (Tz / float(Core_TCMB_K)) * (ratio - 1.0)

    # Physical domain: beta^2 >= 0 and beta < 1
    beta2 = np.where(np.isfinite(beta2), beta2, np.nan)
    beta2 = np.clip(beta2, 0.0, 1.0 - 1e-12)

    return beta2

def v_from_modified_friedmann(
    z,
    E_values,
    Omega_m0: float | None = None,
    T_model=None,
    ...
    """
    Infer v(z) directly from the modified Friedmann relation.

        v(z) = c * sqrt(beta^2(z))

    This is not a free parameter and not an imposed velocity law.
    """
    beta2 = beta2_from_modified_friedmann(
        z,
        E_values,
        Omega_m0=Omega_m0,
        T_model=T_model,
    )
    return float(Core_c_light) * np.sqrt(beta2)

def Gab_from_modified_friedmann(
    z,
    E_values,
    Omega_m0: float | None = None,
    T_model=None,
    ...
    """
    Reconstruct Gab(z) using v(z) inferred from the modified Friedmann relation.
    """
    z = np.asarray(z, dtype=float)
    Tz = T_cosmic_z(z) if T_model is None else np.asarray(T_model(z), dtype=float)

    vz = v_from_modified_friedmann(
        z,
        E_values,
        Omega_m0=Omega_m0,
        T_model=T_model,
...
def Omega_DE_eff_from_E(
    z,
    E_values,
    Omega_m_ref: float = Core_OMEGA_M_LCDM,
):
    """
    Effective dark-energy-like component inferred from a flat-FRW comparison:

        Omega_DE_eff(z) = E^2(z) - Omega_m_ref * (1+z)^3

    This is a diagnostic quantity, not an assumed cosmological constant.
    """
    z = np.asarray(z, dtype=float)
    E_values = np.asarray(E_values, dtype=float)

    return E_values ** 2 - float(Omega_m_ref) * (1.0 + z) ** 3
...
def w_eff_from_E(
    ...
    """
    Effective equation of state reconstructed from E(z):

        w_eff(z) = -1 + (1+z)/3 * d ln Omega_DE_eff(z) / dz

    Valid where Omega_DE_eff(z) > 0.
    """
    z = np.asarray(z, dtype=float)
    E_values = np.asarray(E_values, dtype=float)

    Omega_eff = Omega_DE_eff_from_E(
        z,
        E_values,
        Omega_m_ref=Omega_m_ref,
    )

    Omega_eff_safe = np.where(Omega_eff > 0.0, Omega_eff, np.nan)

    lnOmega = np.log(Omega_eff_safe)
    dlnOmega_dz = np.gradient(lnOmega, z)

    w_eff = -1.0 + ((1.0 + z) / 3.0) * dlnOmega_dz

    return w_eff

def E_from_H_observed(H_values, H0: float = Core_Hubble_H0):
    """
    Convert observed H(z) into dimensionless E(z):

        E_obs(z) = H_obs(z) / H0
    """
    return np.asarray(H_values, dtype=float) / float(H0)
...
def w_eff_from_Gab(
    ...
    """
    DK-RD2 thermodynamic-gravitational effective equation of state.

    Direct expression in terms of Gab(z), without using Omega_DE_eff:
    ...
    Equivalent form:
        ...
        w_eff^Gab(z) =
            (1+z)/3 * d/dz [ ln(Gab(z)/G0) ]

    Notes
    -----
    - If Gab is constant, w_eff^Gab = 0.
    - This is not the ΛCDM dark-energy equation of state.
    - It measures the effective thermodynamic contribution of the evolving
      gravitational coupling.
    """
    z = np.asarray(z, dtype=float)
    Gab_values = np.asarray(Gab_values, dtype=float)

    mu = Gab_values / float(Core_G0)
    mu_safe = np.where(mu > 0.0, mu, np.nan)

    ln_mu = np.log(mu_safe)
    dlnmu_dz = np.gradient(ln_mu, z)

    return ((1.0 + z) / 3.0) * dlnmu_dz

# Direct DK-RD2 effective equation of state from Gab(z):
#
#   μ(z) = G_ab(z) / G0
#
#   w_eff^Gab(z) =
...
#
#   w_eff^Gab(z) =
#       (1+z)/3 * d/dz [ ln(G_ab(z)/G0) ]
#
# Since:
#
#   G_ab(z)/G0 = 1 + β²(z) * T0/T(z)
#
# then:
#
#   w_eff^Gab(z) =
#       (1+z)/3 * d/dz ln[1 + β²(z) * T0/T(z)]

def w_eff_from_beta2(
    z,
    beta2_values,
    T_model=None,
):
    """
    DK-RD2 w_eff directly from beta^2(z).

    Since:

        Gab/G0 = 1 + beta^2(z) * T0/T(z)

    then:

        w_eff^Gab(z) =
            (1+z)/3 * d/dz ln[1 + beta^2(z) * T0/T(z)]
    """
    z = np.asarray(z, dtype=float)
    beta2_values = np.asarray(beta2_values, dtype=float)

    Tz = T_cosmic_z(z) if T_model is None else np.asarray(T_model(z), dtype=float)

    mu = 1.0 + beta2_values * (float(Core_TCMB_K) / Tz)
    mu_safe = np.where(mu > 0.0, mu, np.nan)
...
def w_eff_from_modified_friedmann_Gab(
    z,
    E_values,
    Omega_m0: float | None = None,
    T_model=None,
    ...
    1. Infer beta^2(z) from the modified Friedmann equation.
    2. Reconstruct Gab(z).
    3. Compute w_eff directly from Gab(z).

    No Omega_DE_eff is used.
    No v(z) ansatz is introduced.
    """
    z = np.asarray(z, dtype=float)
    E_values = np.asarray(E_values, dtype=float)
    ...
    Gab_values = Gab_from_modified_friedmann(
        z,
        E_values,
        Omega_m0=Omega_m0,
        T_model=T_model,
    ...
    w_gab = w_eff_from_Gab(z, Gab_values)

    return w_gab, Gab_values, beta2
...
def dkrd2_to_csv(
    ...
    *,
    ...
    try:
        header.append(f"# repository: {Core_autor_text}")
    except Exception:
        pass
    try:
...
"""
####################################################################################
#### what is gravity rutines
#### Last Revision: May/2026
####################################################################################
"""
# --- BAO / sound-horizon reference constants ---
Core_rd_planck_mpc = 147.09
Core_rd_fit_bounds_mpc = (120.0, 170.0)
Core_alpha_DK_bounds = (0.0, 1.0)

# --- DESI DR1 cosmic chronometer reference point ---
Core_DESI_CC_z = 0.12
Core_DESI_CC_H = 71.33
Core_DESI_CC_H_err = 4.20
...
# =============================================================================
# DESI DR2 Gaussian BAO helpers
# =============================================================================

def load_desi_gaussian_bao(mean_path: str, cov_path: str):
    """
    Load DESI Gaussian BAO compressed data.
    ...
    data = np.genfromtxt(
        mean_path,
        comments="#",
        dtype=[("z", float), ("value", float), ("quantity", "U32")],
        encoding="utf-8",
    )
    ...
    if cov.shape != (len(values), len(values)):
        raise ValueError(
            f"BAO covariance shape {cov.shape} does not match mean vector length {len(values)}"
        )

    return z, values, quantity, cov, cov_inv


def bao_distance_vector_over_rs(
    z,
    quantity,
    E_function,
    Core_H0: float,
    ...
    **E_kwargs,
    ...
    """
    Build model vector in same order as DESI Gaussian BAO mean file.

    Observables:
        DM_over_rs = D_M(z) / r_d
        DH_over_rs = D_H(z) / r_d = c / [H(z) r_d]
        DV_over_rs = [z D_M^2 D_H]^(1/3) / r_d
    """
    z = np.asarray(z, dtype=float)
    ...
    for zi, qi in zip(z, quantity):
        DM = comoving_distance(
            zi,
            E_function,
            Core_H0,
            Core_c_km_s=Core_c_km_s,
        ...
        Ez = float(E_function(zi, **E_kwargs))
        Hz = float(Core_H0) * Ez
        DH = float(Core_c_km_s) / Hz

        if qi == "DM_over_rs":
            out.append(DM / rd_mpc)

        elif qi == "DH_over_rs":
            out.append(DH / rd_mpc)

        elif qi == "DV_over_rs":
            DV = (zi * DM**2 * DH) ** (1.0 / 3.0)
            out.append(DV / rd_mpc)

        else:
            raise ValueError(f"Unknown DESI BAO quantity: {qi}")

    return np.asarray(out, dtype=float)


def chi2_gaussian_bao(model_vector, obs_vector, cov_inv):
    """
    Gaussian BAO chi-square.
    """
    delta = np.asarray(model_vector, dtype=float) - np.asarray(obs_vector, dtype=float)
...
def fit_rd_for_bao(
    ...
    Core_H0: float,
    *,
    rd_bounds=(120.0, 170.0),
    Core_c_km_s: float = Core_c_km_s,
    **E_kwargs,
):
    """
    Fit r_d for fixed background E(z).
    """
    from scipy.optimize import minimize_scalar

    def chi2_of_rd(rd):
        model = bao_distance_vector_over_rs(
            z,
            quantity,
            E_function,
            Core_H0,
        ...
        return chi2_gaussian_bao(model, values, cov_inv)
    ...
    best_model = bao_distance_vector_over_rs(
        z,
        quantity,
        E_function,
        Core_H0,
    ...
    pulls = (best_model - values) / np.sqrt(np.diag(np.linalg.inv(cov_inv)))

    return {
        "best_rd": best_rd,
        "best_chi2": best_chi2,
        "chi2_per_point": best_chi2 / len(values),
        "model": best_model,
        "residuals": best_model - values,
        "pulls": pulls,
    }

def chi2_cosmic_chronometer_point(
    H_model_value: float,
    H_obs: float = 71.33,
    sigma_H: float = 4.20,
):
    """
    DESI DR1 cosmic chronometer point from Wang, Lei & Fan 2026:
        H(z=0.12) = 71.33 ± 4.20 km/s/Mpc
    """
    return float(((H_model_value - H_obs) / sigma_H) ** 2)
...
if __name__ == "__main__":
    ...
    print("E_LCDM(0):", E_LCDM(0.0,
        Core_Omega_m=Core_OMEGA_M_LCDM,
        Core_Omega_L=Core_OMEGA_L_LCDM
    ))


    print("E_DK_raw(0):", E_Relativistic(0.0,
        Core_Omega_m=None,
        Omega_L_value=None,
        v_model=None,
        T_model=None
    ...
    z_test = np.array([0.0, 1e-5, 1e-3])

    print("E_DK near zero:", E_Relativistic(
        z_test,
        Core_Omega_m=None,
        Omega_L_value=None,
        v_model=None,
